...
**Note on `toModelMessage()` and built-in compaction:** The existing error filter and `toModelMessage()` conversion remain in place *after* the hook. Upstream OpenCode's `toModelMessage()` has no knowledge of archive placeholders or message ID prefixing — those are Context Bonsai features that exist only in the fork. The plugin must handle all archive rendering (filtering archived messages, injecting summary placeholders, prefixing message IDs) itself in the `chat.context` callback; `toModelMessage()` then converts the plugin's output to model format. If Context Bonsai prunes context effectively via this hook, the token counts reported by the API will stay within limits, and the built-in overflow compaction at `prompt.ts:555-570` won't trigger — it acts as a safety net, not a conflict.
